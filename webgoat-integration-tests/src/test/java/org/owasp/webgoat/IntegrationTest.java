...
public abstract class IntegrationTest {
  ...
  @Getter private String webgoatUser = UUID.randomUUID().toString();
  ...
  public static void beforeAll() {
    ...
    if (!started) {
      ...
      if (!isAlreadyRunning(WG_PORT)) {
        SpringApplicationBuilder wgs =
            new SpringApplicationBuilder(StartWebGoat.class)
                .properties(
                    Map.of(
                        "spring.config.name",
                        "application-webgoat,application-inttest",
                        "WEBGOAT_SSLENABLED",
                        WG_SSL,
                        "WEBGOAT_PORT",
                        WG_PORT));
      ...
      if (!isAlreadyRunning(WW_PORT)) {
        SpringApplicationBuilder wws =
            new SpringApplicationBuilder(WebWolf.class)
                .properties(
                    Map.of(
                        "spring.config.name",
                        "application-webwolf,application-inttest",
                        "WEBWOLF_PORT",
                        WW_PORT));
        wws.run();
      }
  ...
  /**
   * Helper method for most common type of test. POST with parameters. Checks for 200 and
   * lessonCompleted as indicated by expectedResult
   *
   * @param url
   * @param params
  ...
  /**
   * Helper method for most common type of test. PUT with parameters. Checks for 200 and
   * lessonCompleted as indicated by expectedResult
   *
   * @param url
   * @param params
