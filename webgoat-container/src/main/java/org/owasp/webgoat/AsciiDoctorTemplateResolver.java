...
public class AsciiDoctorTemplateResolver extends FileTemplateResolver {
  ...
  /**
   * The resource name is for example HttpBasics_content1.adoc. This is always located in the
   * following directory: <code>plugin/HttpBasics/lessonPlans/en/HttpBasics_content1.adoc</code>
   */
  ...
  private InputStream readInputStreamOrFallbackToEnglish(String resourceName, Language language) {
    InputStream is =
        Thread.currentThread()
            ...
            .getResourceAsStream(
                computeResourceName(resourceName, language.getLocale().getLanguage()));
    if (is == null) {
      is =
          Thread.currentThread()
              .getContextClassLoader()
              .getResourceAsStream(computeResourceName(resourceName, "en"));
    }
    return is;
  }
