...
public class LabelService {
  ...
  /**
   * We use Springs session locale resolver which also gives us the option to change the local later
   * on. For now it uses the accept-language from the HttpRequest. If this language is not found it
   * will default back to messages.properties.
   *
   * <p>Note although it is possible to use Spring language interceptor we for now opt for this
   * solution, the UI will always need to fetch the labels with the new language set by the user. So
   * we don't need to intercept each and every request to see if the language param has been set in
   * the request.
   *
