...
public class WebWolf {
  ...
  public static void main(String[] args) {
    ...
    String webGoatHost =
        null == System.getenv("WEBGOAT_HOST") ? "127.0.0.1" : System.getenv("WEBGOAT_HOST");
    String webWolfHost =
        null == System.getenv("WEBWOLF_HOST") ? "127.0.0.1" : System.getenv("WEBWOLF_HOST");
    ...
    if (null == fileEncoding || !fileEncoding.equals("UTF-8")) {
      System.out.println(
          "It seems the application is startd on a OS with non default UTF-8 encoding:"
              + fileEncoding);
    ...
    if (!isAlreadyRunning(webGoatHost, dbPort)) {
      System.out.println(
          "It seems that the required database is not running. Please start WebGoat with the"
              + " integrated or standalone database first.");
      System.exit(-1);
    }
